...
par(mfrow = c(1,2))
matplot(t(nbp_ens), type = 'l', ylab = 'GtC/year', col = plotcol, ylim = c(-10, 10))
...
nbp_accum_ix <- which(nbp_modern_accum_level1a > -106 &  nbp_modern_accum_level1a <16.3)
...
matplot(years, t(cumulative_nbp), type = 'l', lty = 'solid',ylim = ylim, col = plotcol,
        ...
        bty = 'n')
matlines( years, t((cumulative_nbp[toplevel_to_level_1a_ix, ])[nbp_accum_ix, ]),col = nbpcol,lwd = 1.5, lty = 'solid')
matlines( years, t((cumulative_nbp[toplevel_to_level_1a_ix, ])[level2_ix, ]),col = awcol,lwd = 1.5, lty = 'solid')

legend('topleft', legend  = c('Cumulative NBP constraint', 'AW constraint'), col = c(nbpcol, awcol) , lty = 'solid', lwd = 1.5)
#dev.off()
...
## Total land carbon change (incorrect)
...
# This calculates total land carbon incorrectly
