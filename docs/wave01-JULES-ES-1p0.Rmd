...
createConstraintString <- function(yvec, mins, maxes){
  # This function constructs the string to be evaluated
  out <- 'which('
...
inputConstraintSize <- function(X_unif, Y_unif, yvec, mins, maxes){
  
  ix_kept <- eval(parse(text = createConstraintString(yvec=yvec, mins = mins, maxes = maxes)))
  
  prop_kept <- length(ix_kept) / nrow(X_unif)
...
for(i in 1:ncol(Y_pred)){
  ...
  ics <- inputConstraintSize(X_unif = X_unif,
                             Y_unif = subset(Y_pred, select=yvec ),
                             yvec = yvec,
...
axis(1, labels = colnames(Y_pred), at = 1:9, las = 2)
