...
library(imptools)
library(viztools)
X_unif <- samp_unif(200000, mins = rep(0,32), maxes = rep(1, 32))
colnames(X_unif) <- colnames(X)

pred_unif <- predict.km(fit_ag_me, newdata = X_unif, type = 'UK' ) 

# Keeps about 20%
X_kept_ix <- which(abs(pred_unif$mean) < abs(ag_modern_me_stan))
