...
```{r plot-carbon-cycle-timeseries-primary, fig.width = 10, fig.height = 12, eval = FALSE }
...
panel.hist = function(x, ...) {
  ...
  hist(x, freq = FALSE, col="grey", add=TRUE, main = '') 
  lines(density(x))
}
...
```{r,fig.width = 12, fig.height = 12, warning = FALSE, eval = FALSE}
...
ens_wave01_frac_mean_mv <- makeJulesEnsembleModernValue(ensloc = ensloc_wave01, 
...
pdf(width = 7, height = 6, file = 'figs/land_surface_jules_vs_CCI.pdf')
par(mar = c(10,4,2,1))
df <- barplot(rbind(frac_cci*100, frac_mv), beside = TRUE, las = 2, ylim = c(0,100), 
              ylab = c("Global mean land surface fraction (%)"),
              names.arg = fraclab,
        col = wes_palette('Zissou1')[c(2,3)])

arrows(df[1,], frac_cci_min*100,df[1,],  frac_cci_max*100, angle = 90, code = 3, length = 0.05)
n_in <- nrow(Y_frac_level1a_wave01)

points(rep(df[2,1], n_in) + rnorm(n_in, mean = 0, sd = 0.1), Y_frac_level1a_wave01[,'treeFrac_lnd_mean'],
       col = makeTransparent('black', 20), pch = 20)

points(rep(df[2,2], nrow(Y_frac_level1a_wave01)) + rnorm(n_in, mean = 0, sd = 0.1), Y_frac_level1a_wave01[,'c3PftFrac_lnd_mean'],
       col = makeTransparent('black', 20), pch = 20)

points(jitter(rep(df[2,3], nrow(Y_frac_level1a_wave01))) + rnorm(n_in, mean = 0, sd = 0.1), Y_frac_level1a_wave01[,'c4PftFrac_lnd_mean'],
       col = makeTransparent('black', 20), pch = 20)

points(jitter(rep(df[2,4], nrow(Y_frac_level1a_wave01)))+ rnorm(n_in, mean = 0, sd = 0.1), Y_frac_level1a_wave01[,'shrubFrac_lnd_mean'],
       col = makeTransparent('black', 20), pch = 20)

points(jitter(rep(df[2,5], nrow(Y_frac_level1a_wave01))) + rnorm(n_in, mean = 0, sd = 0.1), Y_frac_level1a_wave01[,'baresoilFrac_lnd_mean'],
       col = makeTransparent('black', 20), pch = 20)

legend('top', fill = c(wes_palette('Zissou1')[c(2,3)], NA), border = NA, legend = c('LC CCI+- 50%', 'standard JULES-ES-1.0', 'ensemble JULES-ES-1.0'), col = c(NA, NA, 'black'), pch = c(NA, NA,20))
dev.off()
#dev.off()
...
treefrac_ix_em <- which(Y_frac_pred_unif_mean[, 'treeFrac_lnd_mean'] > frac_cci_min[1]*100 &
                          Y_frac_pred_unif_mean[, 'treeFrac_lnd_mean'] < frac_cci_max[1]*100)
...
X_treefrac <- rbind(X_unif[treefrac_ix_em, ], X_stan_norm)
colnames(X_treefrac) <- 1:32
...
# Put all of the constraint inputs together
Y_pred <- cbind(Y_const_pred_unif_mean, Y_frac_pred_unif_mean)

Y_pred_mins <- c(0, 35, 750, 300, (frac_cci_min*100))
Y_pred_maxes <- c(10000, 80, 3000, 800, (frac_cci_max*100))
...
ics_individual_list <- vector(mode = 'list', length = ncol(Y_pred))

for(i in 1:ncol(Y_pred)){
  
  yvec <- colnames(Y_pred)[i]
  ics <- inputConstraintSize(Y_unif = subset(Y_pred, select=yvec ),
                             yvec = yvec,
                             mins = Y_pred_mins[i],
                             maxes = Y_pred_maxes[i]
  )
  
  ics_individual_list[[i]] <- ics
  
}


perc_kept_individual <- sapply(ics_individual_list, FUN = function(x) x$prop_kept) * 100
...
to_100 <- 100 - perc_kept_individual
percmat <- rbind(perc_kept_individual, to_100)

colnames(percmat) <- colnames(Y_pred)

#pdf(width = 7, height = 7, file = 'figs/individual_constraint.pdf')
par(mar = c(14, 4, 2, 1), las = 1)
barplot(percmat, ylim = c(0,100), xlab = '',
     ylab = 'Proportion of input space retained (%)', col = c(zissou5[2], 'grey90'),
     las = 2)
...
ics_list <- vector(mode = 'list', length = ncol(Y_pred))

for(i in 1:ncol(Y_pred)){
  
  yvec <- colnames(Y_pred)[1:i]
  ics <- inputConstraintSize(Y_unif = subset(Y_pred, select=yvec ),
                             yvec = yvec,
                             mins = Y_pred_mins[1:i],
                             maxes = Y_pred_maxes[1:i]
  )
...
perc_kept <- sapply(ics_list, FUN = function(x) x$prop_kept) * 100

perc_inv <- 100 - perc_kept

perc_seq_mat <- rbind(perc_kept, perc_inv)
colnames(perc_seq_mat) <- colnames(Y_pred)

# #pdf(file = 'figs/sequential_constraint.pdf')
# par(mar = c(15, 4, 2, 1), las = 1)
# plot(1:9, perc_kept, ylim = c(0,100), type = 'b', axes = FALSE, xlab = '',
#      ylab = 'Proportion of input space retained (%)')
# 
# axis(2)
# axis(1, labels = paste0(colnames(Y_pred), ' (',round(Y_pred_mins,1),'-', round(Y_pred_maxes,1),')'), at = 1:9, las = 2)
# #dev.off()

#pdf(file = 'figs/sequential_constraint.pdf', width = 8, height = 8)
par(mar = c(14, 4, 2, 1), las = 1)
barplot(perc_seq_mat, ylim = c(0,100), xlab = '',
     ylab = 'Proportion of input space retained (%)', col = c(zissou5[2], 'grey90'),
     las = 2)
...
X_all_constraints <- rbind(X_unif[ics_list[[8]]$ix_kept , ], X_stan_norm)
colnames(X_treefrac) <- 1:32


#pdf(file = 'X_treefrac_pairs_density.pdf', width = 10, height = 10)
pairs(X_all_constraints,
    #  labels = 1:d,
      gap = 0, lower.panel = NULL, xlim = c(0,1), ylim = c(0,1),
      #panel = dfunc_up_truth,
      #diag.panel = panel.hist,
