...
#loostats_km_Y_level1a <- vector(mode = 'list', length = length(y_names_sum))

#for(i in 1:length(emlist_km_Y_level1a)){
  
#  loostats <- kmLooStats(emlist_km_Y_level1a[[i]])
#  loostats_km_Y_level1a[[i]] <- loostats
#  print(loostats$pmae)
#}
...
pred_level2_ix <- which(pred_km_Y_const_level1a_scaled[,'nbp_lnd_sum'] > 0 &
                    pred_km_Y_const_level1a_scaled[,'npp_nlim_lnd_sum'] > 35 &  pred_km_Y_const_level1a_scaled[,'npp_nlim_lnd_sum'] < 80 &
                    pred_km_Y_const_level1a_scaled[,'cSoil_lnd_sum'] > 750 & pred_km_Y_const_level1a_scaled[,'cSoil_lnd_sum'] < 3000 &
                  pred_km_Y_const_level1a_scaled[,'cVeg_lnd_sum'] > 300 & pred_km_Y_const_level1a_scaled[,'cVeg_lnd_sum'] < 800
...
```{r}
loo_m_stan <- leaveOneOut.km(m_stan, type = 'UK')

loo_m_logy <- leaveOneOut.km(m_logy, type = 'UK')

loo_m_sqrty <- leaveOneOut.km(m_sqrty, type = 'UK')
...
##  
```{r}
test <- matrix(1:150, ncol = 3)
colnames(test) <- c('col1', 'col2', 'col3')
Xrange <- matrix(c(20, 100, 80, 100, 120, 147), ncol = 3, nrow = 2, byrow = FALSE)
colnames(Xrange) <- c('col1', 'col2', 'col3')

kept_list <-vector(mode = 'list', length = ncol(test))

for(i in 1:3){
  
  kept_ix <- which(test[ ,i] > Xrange[1, i] & test[ ,i] < Xrange[2, i])
  print(kept_ix)
...
test_ix <- withinConstraints(Y_const_level1a_scaled, AW_const)


withinConstraints(pred_km_Y_const_level1a_scaled_cVeg_logy, AW_const)
