...
lhs_min <- apply(lhs, 2, min)
lhs_max <- apply(lhs,2, max)
...
for(i in 1:length(y_names_sum)){
  ...
  yname <- y_names_sum[i]
  y <- Y_level1a[, yname]
  oat <- oaatSensvarKm(X = X_level1a, y = y, med = FALSE, hold = X_standard_norm)
...
```{r}
fit_list_Y_level1a <- createKmFitList(X = X_level1a, Y = Y_level1a)
```
...
X_fast <- fast99(model = NULL, factors = colnames(X_level1a), n = 3000,
                 q = "qunif", q.arg = list(min = 0, max = 1))

fast_tell_list <- vector(mode = 'list', length = ncol(Y_level1a))

for(i in 1:ncol(Y_level1a)){
  
  fit <- fit_list_Y_level1a[[i]]
...
for(i in 1:length(fast_tell_list)){
  par(las = 2, mar = c(9,5,3,2))
  barplot(bp_convert(fast_tell_list[[i]]), col = c('skyblue', 'grey'), ylab = 'relative sensitivity', main = 'FAST99 Sensitivity')
}
...
FAST_total <- matrix(nrow = length(fast_tell_list), ncol = d)

for(i in 1:length(fast_tell_list)){
 
  # sum the direct effect and interaction terms to get a total
  FAST_total[i, ] <- apply(bp_convert(fast_tell_list[[i]]),2,sum)
}
...
colnames(FAST_total) <- colnames(X_level1a)
rownames(FAST_total) <- colnames(Y_level1a)
#pdf(file = 'figs/FAST_sensmat_level1a.pdf', width = 7, height = 8)
oaatSensvarSummaryPlot(FAST_total)
#dev.off()
...
MF_YAnom <- multiFAST(X = X_level1a, Y = YAnom_level1a)
...
FAST_total_YAnom <- matrix(nrow = length(MF_YAnom$fast_tell_list), ncol = d)

for(i in 1:length(MF_YAnom$fast_tell_list)){
 
  # sum the direct effect and interaction terms to get a total
  FAST_total_YAnom[i, ] <- apply(bp_convert(MF_YAnom$fast_tell_list[[i]]),2,sum)
}

colnames(FAST_total_YAnom) <- colnames(X_level1a)
rownames(FAST_total_YAnom) <- colnames(Y_level1a)
#pdf(file = 'figs/FAST_sensmat_level1a.pdf', width = 7, height = 8)
sensMatSummaryPlot(FAST_total_YAnom)
#dev.off()
...
pdf(file = 'figs/FAST_sensmat_Y_level1a.pdf', width = 7, height = 8)
sensMatSummaryPlot(FAST_total)
dev.off()

pdf(file = 'figs/FAST_sensmat_YAnom_level1a.pdf', width = 7, height = 8)
sensMatSummaryPlot(FAST_total_YAnom)
dev.off()
...
sensrank_Y_level1a_oat <- SensRank(oat_var_sensmat_level1a_Y)
sensrank_YAnom_level1a_oat <- SensRank(oat_var_sensmat_level1a_YAnom)

sensrank_FAST <- SensRank(FAST_total)
sensrank_FAST_YAnom <- SensRank(FAST_total_YAnom)

  
sens_ranks <- cbind(sensrank_Y_level1a_oat$rank,sensrank_FAST$rank,  sensrank_YAnom_level1a_oat$rank, sensrank_FAST_YAnom$rank )
colnames(sens_ranks) <- c('OAT_modern_value', 'FAST_modern_value', 'OAT_anomaly', 'FAST_anomaly')
